...
output_dir = f'/root/autodl-fs/output_grad/{path.split('/')[-1]}'
...
for iprompt in prompt:
    ...
    input_ids = tokenizer(iprompt, return_tensors="pt", add_special_tokens=True).input_ids.to(model.device)
...
torch.save(os.paht.join(output_dir, 'gemma_lrp.pt'))
